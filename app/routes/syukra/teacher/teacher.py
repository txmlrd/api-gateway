...
def upload_file_pembelajaran():
    ...
    try:
        response = requests.post(f"{Config.URL_CONTENT}/item-pembelajaran", files=file)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.RequestException as e:
        return jsonify({"error": "Class Content Service unavailable", "details": str(e)}), 503
...
def create_weekly_section():
    ...
    try:
        response = requests.post(f"{Config.URL_CLASS_CONTROL}/teacher/kelas/weekly-section", json=data)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.RequestException as e:
        return jsonify({"error": "Class Control Service unavailable", "details": str(e)}), 503
...
def upload_file_assignment():
    file = request.files.get('file')
    try:
        response = requests.post(f"{Config.URL_CONTENT}/item-pembelajaran/assignment", files=file)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.RequestException as e:
        return jsonify({"error": "Class Content Service unavailable", "details": str(e)}), 503
...
def create_assignment():
    ...
    try:
        response = requests.post(f"{Config.URL_CLASS_CONTROL}/teacher/kelas/assignment", json=data)
        return jsonify(response.json()), response.status_code
    except requests.exceptions.RequestException as e:
        return jsonify({"error": "Class Control Service unavailable", "details": str(e)}), 503
